package ca.esdot.picshop.editors
{
	...
	public class BordersEditor extends AbstractEditor
	{
		...
		public function BordersEditor(editView:EditView, filterType:String) {
			super(editView);
		...
		override protected function updateChildrenLayout():void {
			...
			if(filterType == ImageBorders.THIN || filterType == ImageBorders.THICK || 
			   filterType == ImageBorders.AURA || filterType == ImageBorders.FEATHERED){	
				...
				childrenContainer.addChild(slider);
				...
				childrenContainer.addChild(colorButton);
		...
		override public function init():void {
			...
			switch(filterType){
				...
				case ImageBorders.POLAROID_DIRTY:
					...
					bottomText.y = imageView.container.y + editView.marginTop + currentBitmap.height * .8;
		...
		protected function applyBorder(data:BitmapData, source:Boolean = false):void {
			switch(filterType){
				...
				case ImageBorders.THIN:
					ImageBorders.border(data, slider.position, currentColor);
					break;
				
				case ImageBorders.THICK:
					ImageBorders.border(data, slider.position, currentColor, .1);
					break;
				
				case ImageBorders.AURA:
					if(source){
						ImageBorders.border(data, .5, currentColor, slider.position * .2, 1);
					} else {
						ImageBorders.border(data, .5, currentColor, slider.position * .2, 2);
					}
					break;
				
				case ImageBorders.FEATHERED:
					ImageBorders.border(data, slider.position, currentColor, .065, 0.02);
					break;
