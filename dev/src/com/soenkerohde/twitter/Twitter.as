package com.soenkerohde.twitter {
	...
	public class Twitter extends EventDispatcher implements ITwitter {
		...
		public static const SET_STATUS:String = "http://twitter.com/statuses/update.json";
		public static const SET_STATUS_WIDTH_MEDIA:String = "http://upload.twitter.com/1/statuses/update_with_media.format";
		...
		public function setStatus( accessToken : OAuthToken, status : String, media:FileReference = null ) : void {
			// create OAuthRequest
			if(!media){
				var oauthRequest:OAuthRequest = new OAuthRequest( "POST", SET_STATUS, { status: status }, consumer, accessToken );
			} else {
				var oauthRequest:OAuthRequest = new OAuthRequest( "POST", SET_STATUS_WIDTH_MEDIA, { status: status }, consumer, accessToken );
			}
